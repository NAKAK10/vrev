...
async function focusTarget(pagePath, anchor, restoreContext) {
  ...
  try {
    ...
    if (frame) {
      ...
      else if (anchor?.bounds && doc?.defaultView) { const size = documentSize(doc); doc.defaultView.scrollTo({ left: Math.max(0, anchor.bounds.x * size.width - doc.defaultView.innerWidth / 2), top: Math.max(0, anchor.bounds.y * size.height - doc.defaultView.innerHeight / 2), behavior: "smooth" }); focused = true; }
...
function redrawMarks() {
  ...
  const add = (bounds, index, className = "") => { const mark = element("div", `vr-annotation-mark ${className}`); mark.style.left = `${bounds.left}px`; mark.style.top = `${bounds.top}px`; mark.style.width = `${Math.max(2, bounds.width)}px`; mark.style.height = `${Math.max(2, bounds.height)}px`; if (index) { const pin = element("span", "vr-annotation-pin"); pin.textContent = String(index); mark.append(pin); } layer.append(mark); };
  for (const [index, annotation] of (layer.__marks || []).entries()) { if (annotation.issue_state) continue; const currentPath = frame ? pagePathForFrame(stage, frame) : stage.__target?.entry_path; if (annotation.page_path && String(annotation.page_path).replace(/^\//, "") !== String(currentPath).replace(/^\//, "")) continue; try { let box = null; if (frame && annotation.kind === "dom") { const selected = annotation.anchor?.selector ? frame.contentDocument?.querySelector(annotation.anchor.selector) : null; if (selected) { const rect = selected.getBoundingClientRect(); const frameRect = frame.getBoundingClientRect(); box = { left: frameRect.left - stageRect.left + rect.left, top: frameRect.top - stageRect.top + rect.top, width: rect.width, height: rect.height }; } } else if (frame && annotation.anchor?.bounds) { const doc = frame.contentDocument; const win = frame.contentWindow; const size = documentSize(doc); const frameRect = frame.getBoundingClientRect(); const b = annotation.anchor.bounds; box = { left: frameRect.left - stageRect.left + b.x * size.width - win.scrollX, top: frameRect.top - stageRect.top + b.y * size.height - win.scrollY, width: b.width * size.width, height: b.height * size.height }; } else if (image && annotation.anchor?.bounds) { const rect = image.getBoundingClientRect(); const b = annotation.anchor.bounds; box = { left: rect.left - stageRect.left + b.x * rect.width, top: rect.top - stageRect.top + b.y * rect.height, width: b.width * rect.width, height: b.height * rect.height }; } if (box) add(box, index + 1, annotation.id === layer.__selectedId ? "is-selected" : ""); } catch {}
  }
