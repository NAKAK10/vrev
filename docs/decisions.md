...
Disabling `annotation-workflow` unmounts its own AI/annotation/history contribution without deleting review data; the independent GitHub Issue sidebar remains available. AI packageだけがCLI選択と外部AIコマンドの登録・検証・実行を扱う。feature packageは用途に必要なAI modeだけを`ai/v1`へ指定し、利用者へAI選択を求めず、raw command templateも受け取らない。
...
最大並列数と自動実行は通常画面から除外し、AI欄右上のmenuから左上共通設定導線と同じ`/settings/plugins#annotation-workflow`へ遷移させる。CLI選択と外部AIコマンド管理は`/settings/plugins#ai`でAI packageが提供する。
AI packageはOpenCode・Claude・Codex・GitHub Copilot・PiのCLI adapterと、複数の外部AIコマンドを所有する。browserとfeature packageには選択済みmethodの実装詳細を返さない。
...
Root and six-feature-package publication is non-atomic. Because the root package bundles compatible default server/UI copies, one feature package's registry failure does not break initial startup. Recovery is to retain the published root, rerun publication after fixing the failed package, and let the workflow skip package versions already present. Never republish or overwrite an existing version.
...
注釈本文・anchor・source hashの検証、atomic保存、status遷移は`review` pluginが所有し、job enqueue・recovery・coordinator policyは`annotation-workflow` pluginがcapability経由で所有する。Coreはgeneric lifecycle、bridge、process制限だけを担当する。保存後にどのeventで自動実行を予約するか、何msまとめるか、設定UIへどの並列範囲・自動実行toggleを表示するかは`annotation-workflow`の運用policyである。CLI選択と外部AIコマンド設定はAI packageだけが表示する。Coreは任意HTML・remote scriptを受け取らず、検証済みJSON documentを描画する。manifestで明示されたlocal `browser_module`だけはtrusted UI runtimeとしてmount/unmountする。`annotation-workflow`が無効・利用不能なときはAI一括修正・注釈・履歴を含む右sidebar全体を表示せず、review stageを全幅へ広げ、新規enqueueもserver側で拒否する。AI packageを無効化すると登録内容は保持するがAI実行は利用不能になる。plugin設定の保存結果はtoastで通知し、意図的な無効化をload errorとして表示しない。
...
sidebarの現在のstatus/kind filterを通過したannotationだけを、node/regionともcurrent target上へborder＋translucent fillで表示する。右側一覧から除外されたannotationはtargetにも描画しない。状態色は未対応=amber、AI対応中=blue、失敗=red、AI対応済み=green、解決済み=grayとし、node hoverはblue、region previewはpurpleに分離する。live proxyのcurrent pageは`/live/...`ではなく元の`live_url`を基準にcanonical URLへ戻してannotationの`page_path`と比較する。sidebarのannotation card全体、annotation title、「対象を表示」はいずれもtarget focus actionとし、クリックまたはcardへのkeyboard操作でannotation IDを同じ状態色の太いselected markとして強調して対象位置へscrollする。card内の返信入力・button・link等のinteractive controlは本来の操作を優先し、親cardへのclick伝播ではfocusを重複実行しない。annotation card/titleのhoverでは色・下線・背景を変えず、keyboard focusだけ明示する。declarative rendererで`review`の`annotation.create`が成功した直後は、annotation-workflowのserver-owned `workflow-settings`を参照し、`auto_run=true`の場合に同設定の`max_parallel`で`jobs.enqueue`を実行し、AI packageがworkspace設定からmethodを解決する。enqueue完了後にannotations/jobsを再取得して、保存直後のcardを`AI対応中`へ同期する。注釈保存自体は成功済みのため、自動enqueueだけが失敗した場合は保存失敗として扱わず専用error toastを出す。
...
未確認の更新はheaderの通知センターへ集約しない。各packageは自身のSidebar `disclosure`へ安定した`attention_key`とresource由来の`attention_value`を渡し、Baseは閉じているsectionにだけ未確認dotを表示する。sectionを開くとその値をlocalStorageへ既読保存する。この機構はAI job・注釈・履歴・Issueの意味をCoreへ持ち込まない。
