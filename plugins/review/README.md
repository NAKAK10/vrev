...
Vrev のレビュー集約、入力検証、schema v1→v2 migration、active/resolved transaction persistence を所有する bundled default plugin です。
...
`review` pluginはレビュー対象上の汎用`review.overlays` extension pointを宣言します。GitHub Issueはこのpluginのコメントdialogへ挿入されず、`github-issue` packageが専用の選択tool、modal、sidebarを所有します。
